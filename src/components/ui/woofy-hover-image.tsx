...
const WoofyHoverImage: React.FC<WoofyHoverImageProps> = ({
  ...
  const getEffectTypeValue = useCallback((type: string): number => {
    switch (type) {
  ...
  const hexToRgb = useCallback((hex: string): THREE.Color => {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (inside) {
      ...
      const startRadius = uniformsRef.current?.u_radius.value || 0;
      const startTime = Date.now();
  ...
  }, [maskRadius, appearDuration, disappearDuration, onHover, onLeave, disabled]);
